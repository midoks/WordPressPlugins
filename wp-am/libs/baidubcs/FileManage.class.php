...
class FileManage{
	...
	public function __construct(){
		$this->init_config();
		$this->bcs = new BaiduBCS(AM_BCS_AK, AM_BCS_SK);
		$this->ready();
	}
	...
	public function init_config(){
		...
		//AK公钥
		define('AM_BCS_AK', $option['ak']);
		//SK私钥
		define('AM_BCS_SK', $option['sk']);
	...
	public function download($args){
		...
		$acl = array (
			'statements' => array (
				'0' => array (
					'user' 		=> array('*'),
					'resource' 	=> array (BCS_BUCKET.$arg),
					'action' 	=> array ('*'),
					'effect' 	=> BaiduBCS::BCS_SDK_ACL_EFFECT_ALLOW,
					//'referer'	=> array('*'),
		)));
		//解开权限
		$this->bcs->set_object_acl(BCS_BUCKET, $arg, $acl);
		//$info2 = $this->bcs->get_object_acl(BCS_BUCKET, $arg);
		$res = file_get_contents('http://bcs.duapp.com/'.BCS_BUCKET.'/'.$arg);
		//$rep = $this->bcs->get_object(BCS_BUCKET, $arg);
		//加上权限
		$this->set_default_reffer($arg);
		return $res;
	}
